...
function formatDate(value) {
  return value
    ? new Date(value).toLocaleString(
        getCurrentLanguage() === "en" ? "en-US" : "pt-BR",
      )
    : tr("common_never");
}
...
function userRows(users) {
  if (!users.length)
    return `<tr><td colspan="6" class="admin-empty">${tr("admin_users_empty")}</td></tr>`;
  return users
    .map(
      (user) => `
...
async function loadUsers() {
  ...
  document.getElementById("users-list").innerHTML =
    `<div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>${tr("admin_users_name")}</th><th>${tr("admin_users_role")}</th><th>${tr("admin_users_status")}</th><th>${tr("admin_users_last_access")}</th><th>${tr("admin_users_certifications")}</th><th>${tr("admin_users_actions")}</th></tr></thead><tbody>${userRows(users)}</tbody></table></div>`;
}

function requestCards(requests) {
  if (!requests.length)
    return `<p class="admin-empty">${tr("admin_users_requests_empty")}</p>`;
  return requests
    .map((request) => {
      ...
      <p><span class="admin-status ${statusClass}">${escapeHtml(status)}</span> · ${formatDate(request.requested_at || request.created_at)}</p>
...
async function loadRequests() {
  ...
  document.getElementById("requests-list").innerHTML =
    `<div class="admin-list">${requestCards(response.data || [])}</div>`;
}

function friendlyError(error) {
  if (error?.status === 409 || /last active ADMIN/i.test(error?.message || ""))
    return tr("admin_users_last_admin_error");
...
function bindActions() {
  document
    ...
    ?.addEventListener("click", () =>
      loadUsers().catch((error) => showFeedback(error.message, "error")),
    );
  document
    .getElementById("btn-search-users")
    ?.addEventListener("click", () =>
      loadUsers().catch((error) => showFeedback(error.message, "error")),
    );
  document
    .getElementById("btn-load-requests")
    ?.addEventListener("click", () =>
      loadRequests().catch((error) => showFeedback(error.message, "error")),
    );
  document
    ...
    ?.addEventListener("click", async (event) => {
      ...
      const payload = event.target.closest("[data-toggle]")
        ? {
            is_active: !row
              ...
              ?.classList.contains("admin-status-active"),
          }
      ...
      try {
        await window.ValidationAPI.updateUserAccess(id, payload);
        showFeedback(tr("admin_users_updated"));
        await loadUsers();
      } catch (error) {
  ...
  document
    ...
    ?.addEventListener("click", async (event) => {
      ...
      const notes =
        status === "REJECTED"
          ? window.prompt(tr("admin_users_reject"), "")
          : "";
      if (status === "REJECTED" && (!notes || notes.trim().length < 10)) return;
      action.disabled = true;
      try {
        await window.ValidationAPI.reviewValidatorRequest(
          id,
          status,
          notes || "",
        );
        showFeedback(tr("admin_users_request_updated"));
        await loadRequests();
      } catch (error) {
        showFeedback(error.message || tr("admin_users_request_error"), "error");
...
export async function initUsersPage() {
  if (
    !(await initAdminPage({
      roles: ["ADMIN"],
      message: tr("common_unauthorized"),
    }))
  )
    return;
  bindActions();
  try {
    ...
    showFeedback(error.message || tr("admin_users_update_error"), "error");
  }
}
