...
export const LEGACY_PALETTE = Object.freeze(
  [
    ...
    [
      ...
      "Auto Scaling",
      "Compute",
      "https://icon.icepanel.io/AWS/svg/Compute/Auto-Scaling.svg",
    ],
    ...
    [
      ...
      "Amazon ElastiCache",
      "Database",
      "https://icon.icepanel.io/AWS/svg/Database/ElastiCache.svg",
    ],
    ...
    [
      ...
      "Elastic Load Balancing",
      "Networking",
      "https://icon.icepanel.io/AWS/svg/Networking-Content-Delivery/Elastic-Load-Balancing.svg",
    ],
  ...
  return {
    ...
    required_connections: Array.isArray(builder.required_connections)
      ? builder.required_connections
          .filter(
            (connection) =>
              Array.isArray(connection) && connection.length === 2,
          )
...
export function resolveBuilderService(
  slug,
  caseData,
...
export function scoreBuilderServices(selectedSlugs, caseData) {
  ...
  const hasService = (set, slug) =>
    [...set].some((value) => serviceKey(value) === serviceKey(slug));
  const correct = [...selected].filter(
    (slug) => hasService(required, slug) || hasService(optional, slug),
  );
...
export function attachIconFallbacks(container) {
  container
    .querySelectorAll('img[data-service-icon="url"]')
    .forEach((image) => {
...
function serviceInfoForSlug(slug, palette, caseData) {
  return (
    palette.find((service) => serviceKey(service.slug) === serviceKey(slug)) ||
    resolveBuilderService(slug, caseData, palette)
  );
}

export function initArchitectureBuilder({
  caseData,
  legacyPalette = LEGACY_PALETTE,
} = {}) {
  ...
  const setStatus = (message) => {
    ...
    status._builderTimer = setTimeout(() => {
      status.textContent = "";
    }, 3000);
  };
  const updateHint = () => {
  ...
  const showResult = (result) => {
    ...
    document
      .getElementById("builder-phase-banner")
    ...
    circle.classList.add(
      result.score >= 70
        ? "builder-score-circle--pass"
        : "builder-score-circle--fail",
    );
    document.getElementById("builder-score-title").textContent =
      result.score >= 70 ? "Bom trabalho!" : "Precisa melhorar";
    document.getElementById("builder-score-pct").textContent =
      `${result.score}%`;
    document.getElementById("builder-score-sub").textContent =
      `Você identificou ${result.correct.filter((slug) => result.required.has(slug)).length} de ${result.required.size} serviços essenciais.`;
    const renderGroup = (className, icon, title, services) =>
      services.length === 0
        ? ""
        : `
    ...
    const requiredCorrect = result.correct.filter((slug) =>
      result.required.has(slug),
    );
    const optionalCorrect = result.correct.filter((slug) =>
      result.optional.has(slug),
    );
    document.getElementById("builder-result-lists").innerHTML = [
      renderGroup(
        "builder-result-group--correct",
        "fa-solid fa-check",
        "Corretos",
        requiredCorrect,
      ),
      renderGroup(
        "builder-result-group--optional",
        "fa-solid fa-circle-info",
        "Opcionais",
        optionalCorrect,
      ),
      renderGroup(
        "builder-result-group--missing",
        "fa-solid fa-xmark",
        "Faltando",
        result.missing,
      ),
      renderGroup(
        "builder-result-group--extra",
  ...
  const hideResult = () => {
    ...
    document
      .getElementById("builder-result-panel")
  ...
  const verify = () => {
    ...
    const selected = [
      ...new Set(
        nodes
          .map((node) => serviceSlug(node.name || node.data?.svc?.slug))
          .filter(Boolean),
      ),
    ];
  ...
  const init = () => {
    ...
    canvas.addEventListener("drop", (event) => {
      ...
      const x =
        (event.clientX - rect.left) / zoom - drawflowEditor.canvas_x / zoom;
      const y =
        (event.clientY - rect.top) / zoom - drawflowEditor.canvas_y / zoom;
  ...
  container.querySelectorAll(".palette-item").forEach((item) => {
    item.addEventListener("click", () => {
      ...
      const service = serviceInfoForSlug(
        item.dataset.slug,
        palette,
        currentCase,
      );
  ...
  document
    .getElementById("builder-btn-clear")
  ...
  document
    .getElementById("builder-btn-verify")
    ?.addEventListener("click", verify);
  document
    .getElementById("builder-btn-retry")
    ?.addEventListener("click", hideResult);
  document
    .getElementById("builder-btn-export")
    ?.addEventListener("click", () => {
      if (!drawflowEditor) return;
      const blob = new Blob(
        [JSON.stringify(drawflowEditor.export(), null, 2)],
  ...
  document
    .getElementById("tab-btn-builder")
    ?.addEventListener("click", () => setTimeout(init, 50));
  document
    .getElementById("builder-btn-gabarito")
    ?.addEventListener("click", async () => {
      ...
      if (
        ...
        !currentCase?.architecture_graph
      )
        return;
