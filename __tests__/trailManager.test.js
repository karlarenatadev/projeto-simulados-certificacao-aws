...
describe("trailManager e integração da Jornada", () => {
  ...
  test.each(["clf-c02", "saa-c03", "dva-c02", "aif-c01"])(
    "mantém cinco etapas para %s",
    (certId) => {
      expect(TRAILS_BY_CERT[certId]).toHaveLength(5);
      expect(
        TRAILS_BY_CERT[certId].every(
          (stage) => stage.title.pt && stage.title.en,
        ),
      ).toBe(true);
    },
  );

  test("primeira etapa é acessível sem progresso e a segunda exige a primeira", () => {
    expect(isTrailStageUnlocked("clf-c02", "clf-1")).toBe(true);
    expect(isTrailStageUnlocked("clf-c02", "clf-2")).toBe(false);
    expect(isTrailStageUnlocked("clf-c02", "clf-3")).toBe(false);
    expect(isTrailStageUnlocked("clf-c02", "unknown")).toBe(false);

    storageManager.saveGamification({ completedStages: ["clf-1"] }, "clf-c02");
    expect(isTrailStageUnlocked("clf-c02", "clf-2")).toBe(true);
    expect(isTrailStageUnlocked("clf-c02", "clf-3")).toBe(false);
    expect(isTrailStageUnlocked("saa-c03", "saa-2")).toBe(false);
  });

  test("preserva desbloqueios e progresso existentes no refresh sem alterar XP", () => {
    const saved = {
    ...
    storageManager.saveGamification(saved, "clf-c02");
    const before = storageManager.getGamification("clf-c02");

    expect(isTrailStageUnlocked("clf-c02", "clf-2")).toBe(true);
    expect(isTrailStageUnlocked("clf-c02", "clf-3")).toBe(false);
    expect(storageManager.getGamification("clf-c02")).toEqual(before);
  });
  ...
  test.each([
    ...
    for (const otherCertId of Object.keys(TRAILS_BY_CERT).filter(
      (id) => id !== certId,
    )) {
  ...
  test("renderiza recomendação do diagnóstico e estado ativo do Sprint", () => {
    ...
    expect(
      document.querySelector('[data-testid="journey-recommendation"]'),
    ).not.toBeNull();
    expect(
      document.querySelector('[data-testid="journey-sprint-status"]'),
    ).not.toBeNull();
    ...
    expect(
      document.querySelector('[data-recommendation-action="labs"]'),
    ).not.toBeNull();
  });
