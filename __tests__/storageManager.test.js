...
describe("StorageManager - Persistência de Dados", () => {
  ...
  test("Deve persistir marcações da sessão ativa isoladas por usuário e certificação", () => {
    ...
    SessionManager.persist({ user: { id: "user-a" } });
    expect(storage.loadActiveSession("clf-c02").reviewQuestionIds).toEqual([
      "q1",
    ]);
  ...
  test("removeMistake deve marcar erro como resolvido sem apagar historico", () => {
    const question = {
      id: "q1",
      domain: "cloud",
      question: "Qual servico armazena objetos?",
    ...
    expect(storage.getAllMistakes("clf-c02")[0].resolved).toBe(true);
  });
  ...
  test("removeHistoryItem deve recalcular progresso, ultimo resultado e gamificacao", () => {
    ...
    expect(storage.loadLastResult("clf-c02").attemptId).toBe("attempt-regular");
...
describe("review deck personal status", () => {
  test("marks a card mastered and returns it to pending without deleting it", () => {
    localStorage.clear();
    const storage = new StorageManager("test_review_");
    SessionManager.logout();
    storage.addReviewQuestion("clf-c02", {
      id: "deck-q1",
      question: "Question",
      options: ["A"],
      correct: 0,
    ...
    const card = storage.getReviewDeck("clf-c02")[0];
    expect(
      storage.updateReviewStatus("clf-c02", card.questionId, "mastered"),
    ).toBe(true);
    expect(storage.getReviewDeck("clf-c02")[0].reviewStatus).toBe("mastered");
    expect(storage.getReviewDeck("clf-c02")[0].reviewCount).toBe(1);
    storage.updateReviewStatus("clf-c02", card.questionId, "pending");
    expect(storage.getReviewDeck("clf-c02")[0].reviewStatus).toBe("pending");
    expect(storage.getReviewDeck("clf-c02")[0].reviewCount).toBe(2);
  });

  test("does not duplicate the same question id", () => {
    localStorage.clear();
    const storage = new StorageManager("test_review_");
    SessionManager.logout();
    const question = {
    ...
    storage.addReviewQuestion("clf-c02", question);
    storage.addReviewQuestion("clf-c02", question);
    expect(storage.getReviewDeck("clf-c02")).toHaveLength(1);
  });
});
