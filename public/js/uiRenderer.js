...
function _setupModalContainer() {
  ...
  container.className = 'fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity';
  document.body.appendChild(container);
}
...
function _renderModal({ type, payload, onResult }) {
  ...
  card.className = 'bg-slate-900 border border-slate-700 rounded-lg shadow-2xl p-6 max-w-md w-full mx-4 transform scale-95 opacity-0 transition-all';
